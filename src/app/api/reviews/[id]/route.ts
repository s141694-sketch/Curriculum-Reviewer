...
export async function GET(_request: NextRequest, ctx: RouteContext<"/api/reviews/[id]">) {
...
export async function DELETE(_request: NextRequest, ctx: RouteContext<"/api/reviews/[id]">) {
  const { id } = await ctx.params;
  if (isRunning(id)) {
