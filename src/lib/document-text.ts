// Pure text utilities shared by the server (file parsing, pipeline) and the
// browser (which extracts PDF/Word text locally before upload).
...
export class UnsupportedFileError extends Error {
  constructor() {
    super(`صيغة الملف غير مدعومة. الصيغ المدعومة: ${SUPPORTED_EXTENSIONS.join("، ")}`);
  }
}

/**
 * Extract plain text from a curriculum file. Runs on the server and in the
 * browser (which lets large files skip the upload entirely).
 */
export async function extractDocumentText(fileName: string, data: ArrayBuffer): Promise<string> {
  const lower = fileName.toLowerCase();

  if (lower.endsWith(".txt") || lower.endsWith(".md")) {
    return normalizeDocumentText(new TextDecoder("utf-8").decode(data));
  }

  if (lower.endsWith(".docx")) {
    const mammoth = await import("mammoth");
    // Node takes a Buffer; the browser build takes an ArrayBuffer.
    const input =
      typeof window === "undefined" ? { buffer: Buffer.from(data) } : ({ arrayBuffer: data } as unknown as { buffer: Buffer });
  ...
  if (lower.endsWith(".pdf")) {
    ...
    return normalizeDocumentText(text.map(fixVisualArabic).join("\n\n"));
  }
